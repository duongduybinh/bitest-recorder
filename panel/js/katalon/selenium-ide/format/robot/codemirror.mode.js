...
(function(mod) {
  if (typeof exports == "object" && typeof module == "object")
    mod(require("../../../codemirror-5.31.0/lib/codemirror"));
  else if (typeof define == "function" && define.amd)
    define(["../../../codemirror-5.31.0/lib/codemirror"], mod);
  else
    mod(CodeMirror);
})(function(CodeMirror) {
  "use strict";
  ...
  */
  ...
  const RULE_START_INLINE_IF_VAR = r(
    /(\s+)(.*?)(\s+)(=)(\s+)(IF)(\s\|*\s*)(?=[^\s].*\s{2,})/,
    [null, TT.V2, null, TT.OP, null, TT.AM, null],
    { push: 'inline_if_start', sol: true },
  );
  ...
  CodeMirror.defineSimpleMode("robotframework", states);

  CodeMirror.defineMIME("text/x-robotframework", "robotframework");
  console.log("codemirror for robotframework");
});